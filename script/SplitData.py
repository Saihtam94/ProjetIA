...
for file in emailsFileList:
    if not isdir(join(csvsPath, file)):
        ...
        newpath = csvsPath+file[:-4] 
        if not os.path.exists(newpath):
            os.makedirs(newpath)
        x_train_file = open(newpath+"/"+file[:-4] + "_x_train.csv", 'w', newline='')
        x_train_fileWriter = csv.writer(x_train_file, delimiter=',')
        y_train_file = open(newpath+"/"+file[:-4] + "_y_train.csv", 'w', newline='')
        y_train_fileWriter = csv.writer(y_train_file, delimiter=',')
        x_test_file  = open(newpath+"/"+file[:-4] + "_x_test.csv", 'w', newline='')
        x_test_fileWriter  = csv.writer(x_test_file, delimiter=',')
        y_test_file  = open(newpath+"/"+file[:-4] + "_y_test.csv", 'w', newline='')    
